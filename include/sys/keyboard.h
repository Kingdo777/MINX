...
/*                          Macros Declaration                          */
/************************************************************************/
#define	KE_IN_COUNT	32	/* size of keyboard input buffer */
...
typedef struct{
    char    *p_usable;      //指向下一个可用buf
    char    *p_pending;     //指向下一个要处理的code
    int     count;          //缓冲区中的数据
    char    buf[KE_IN_COUNT];//缓冲区
}KB_BUF;
