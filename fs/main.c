 ...
 *****************************************************************************/
/**
 * <Ring 1> Make a available MINX'S FS in the disk. It will
...
void mkfs()
{
	...
	/************************/
	/*       inode map      */
	/*0号Inode保留，1号给了根结点，后面依次给TTY们*/	
	/************************/
	memset(fsbuf, 0, SECTOR_SIZE);
	for (i = 0; i < (NR_TTY + 2); i++)
		fsbuf[0] |= 1 << i;
	...
	pi->i_size = DIR_ENTRY_SIZE * (NR_TTY+1); /* 4 files:
					  * `.',
					  * `dev_tty0', `dev_tty1',
					  */
 ...
 *****************************************************************************/
int rw_sector(int io_type, int dev, uint64_t pos, int bytes, int proc_nr,
		     void* buf)
{
	MESSAGE driver_msg;
